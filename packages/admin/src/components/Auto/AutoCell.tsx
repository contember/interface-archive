import { Component, HasOne, Schema, useEntityPersistSuccess } from '@contember/binding'
import {
	BooleanCell,
	CheckboxField,
	DateCell,
...
const ClickToEdit = Component<{ view: ReactNode, edit: ReactNode }>(
	props => {
		...
		return edit ? <>{props.edit}</> : <div onClick={onClick} ref={ref}>{props.view}</div>
	},
	props => {
		return <>{props.edit}{props.view}</>
	},
)
