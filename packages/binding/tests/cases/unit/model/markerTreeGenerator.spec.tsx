...
describe('Marker tree generator', () => {
	...
	it('combine nested markers', () => {
		...
		const environment = Environment.create()
			.withSchema(new Schema(SchemaPreprocessor.processRawSchema({
				entities: [
					{
						name: 'Foo',
		...
		const outerHasManyEnv = subTreeEnv.withTreeChild({ field: 'hasMany' })
		const hasOneEnv = outerHasManyEnv.withTreeChild({ field: 'hasOne' })
		const innerHasManyEnv = hasOneEnv.withTreeChild({ field: 'common' })
