...
export const EnvironmentMiddleware = Component(({ children, create }: EnvironmentMiddlewareProps) => {
	const env = useEnvironment()
	return (
		<EnvironmentContext.Provider value={create(env)}>
			{children}
...
export const EnvironmentExtensionProvider = Component(<S, R>({ children, extension, state }: EnvironmentWithExtensionProps<S, R>) => {
	const create = useCallback(env => env.withExtension(extension, state), [extension, state])
	return <EnvironmentMiddleware create={create}>{children}</EnvironmentMiddleware>
}, ({ children, extension, state }) => (
	<EnvironmentMiddleware create={env => env.withExtension(extension, state)}>
		{children}
	</EnvironmentMiddleware>
)) as <S, R>(props: EnvironmentWithExtensionProps<S, R>) => ReactElement
