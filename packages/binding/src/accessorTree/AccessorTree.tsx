...
export function AccessorTree({ state, children }: AccessorTreeProps) {
	const stateData = state.name === 'initialized' ? {
		hasUnpersistedChanges: state.data.hasUnpersistedChanges,
		isMutating: state.data.isMutating,
		bindingOperations: state.data.bindingOperations,
	} : {
