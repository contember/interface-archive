		...
		this.eventManager.syncOperation(() => {
			...
			for (const state of StateIterator.eachSiblingRealm(outerState)) {
				...
				for (const targetHasOneMarker of targetHasOneMarkers) {
					...
					if (persistedId instanceof ServerGeneratedUuid && persistedId.value === stateToConnect.entity.id.value) {
						changesDelta-- // It was removed from the list but now we're adding it back.
					} else if (persistedId instanceof ServerGeneratedUuid && persistedId.value === previouslyConnectedState.entity.id.value) {
