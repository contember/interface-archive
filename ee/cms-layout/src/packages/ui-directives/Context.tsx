...
export function useDirectiveProviderRegistry<T extends Record<string, unknown>>(value: T): [SetDirectiveContextType<T>, T] {
	...
	const registerDirective = useEvent(<K extends keyof T>(componentId: string, key: K, value: T[K]) => {
	...
	const updateDirective = useEvent(<K extends keyof T>(componentId: string, key: K, value: T[K]) => {
		const setState = setStateRef.current
	...
	const unregisterDirective = useEvent((componentId: string) => {
		const setState = setStateRef.current
