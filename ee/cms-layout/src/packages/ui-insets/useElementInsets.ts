...
import { nonNegativeNumber, pick } from '../functional'
import { useEvent, useOnElementResize } from '../react-hooks'
...
export function useElementInsets(elementRef: RefObject<HTMLElement>) {
	const { logged } = useConsole('useElementInsets')

	const containerInsets = useContainerInsetsContext()
	const [containerOffsets, setContainerOffsets] = useState<ContainerOffsets>()

	const updateContainerOffsets = useEvent((dimensions: ReturnType<typeof dimensionsFromEntry>) => {
				...
				if (parentElement) {
					...
					if (parentProps.display === 'flex') {
						...
						let offsets = logged('calculated offsets', parentProps.position === 'static' ? {
							offsetBottom: nonNegativeNumber(parentProps.scrollHeight - (parentProps.offsetTop - elementProps.offsetTop) - elementProps.offsetHeight),
							offsetLeft: nonNegativeNumber(elementProps.offsetLeft - parentProps.offsetLeft),
							offsetRight: nonNegativeNumber(parentProps.scrollWidth - (parentProps.offsetLeft - elementProps.offsetLeft) - elementProps.offsetWidth),
							offsetTop: nonNegativeNumber(elementProps.offsetTop - parentProps.offsetTop),
						} : {
							offsetBottom: nonNegativeNumber(parentProps.scrollHeight - elementProps.offsetTop - elementProps.offsetHeight),
							offsetLeft: nonNegativeNumber(elementProps.offsetLeft),
							offsetRight: nonNegativeNumber(parentProps.scrollWidth - elementProps.offsetLeft - elementProps.offsetWidth),
							offsetTop: nonNegativeNumber(elementProps.offsetTop),
						})
