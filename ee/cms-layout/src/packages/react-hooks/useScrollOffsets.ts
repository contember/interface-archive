...
export function useScrollOffsets(refOrElement: RefObjectOrElement<HTMLElement | null> | null, interval: number = 1000 / 60) {
	const { logged } = useConsole('useScrollOffsets')
	...
	const updateScrollOffsetsState = useEvent((offsets: Offsets) => {
	...
	const handler = useEvent(() => {
		const maybeScrollContent = maybeRef(refOrElement) ?? document
