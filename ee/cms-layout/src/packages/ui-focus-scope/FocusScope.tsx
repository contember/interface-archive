...
export function FocusScope({ active = true, children, pressToSkipToLastLabel = 'Skip to last focusable element', pressToSkipToFirstLabel = 'Skip to first focusable element' }: FocusScopeProps) {
	...
	return (
		<Fragment key="focus-scope">
			{active && <button style={visuallyHiddenStyle} ref={firstRef} onFocus={onFocus} onClick={skipToLast}>{pressToSkipToLastLabel}</button>}
			{children}
			{active && <button style={visuallyHiddenStyle} ref={lastRef} onFocus={onFocus} onClick={skipToFirst}>{pressToSkipToFirstLabel}</button>}
		</Fragment>
	)
}
